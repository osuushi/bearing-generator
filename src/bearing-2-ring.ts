...
const retentionVerticalOffset = 0.5;
const retentionTiltOffset = 2;
const minThickness = 1.25;

export class Bearing2Ring {
  ...
  get innerRingPolygon(): Polygon {
    ...
    let outerMin = (this.gapCenterDiameter - this.clearance) / 2;
    let outerMax = outerMin + retentionRadiusOffset;
    return new Polygon([
      // Bottom
      ...
      newXZPoint(
        outerMax,
        this.width - retentionVerticalOffset - retentionTiltOffset
      ),
      // Tilt out
  ...
  get outerRingPolygon(): Polygon {
    const innerMin = (this.gapCenterDiameter + this.clearance) / 2;
  ...
  drawPreview(canvas: HTMLCanvasElement) {
    ...
    const polygons = [innerRing, outerRing];

    const maxX = Math.max(
      ...polygons.map((p) =>
        p.points.reduce((max, p) => Math.max(max, p.x), -Infinity)
      )
    );
    const minX = -maxX;
    const minY = Math.min(
      ...polygons.map((p) =>
        p.points.reduce((min, p) => Math.min(min, p.z), Infinity)
      )
    );
    const maxY = Math.max(
      ...polygons.map((p) =>
        p.points.reduce((max, p) => Math.max(max, p.z), -Infinity)
      )
    );
    ...
    const transform = (p: Point) => {
      return newXZPoint(p.x * scale + translateX, p.z * scale + translateY);
    };
    ...
    const fgGradient = ctx.createLinearGradient(
      upperLeftCorner.x,
      upperLeftCorner.z,
      lowerRightCorner.x,
      upperLeftCorner.z
    ...
    fgGradient.addColorStop(0.5, "rgba(240, 229, 200, 1)");
    ...
    for (const polygon of polygons) {
      const points = polygon.points.map((p) => {
        ...
        return newXZPoint(canvasWidth - transformed.x, transformed.z);
      });
      ctx.beginPath();
      ctx.moveTo(points[0].x, points[0].z);
      for (const point of points.slice(1)) {
  ...
  get fileName(): string {
    ...
    return `bearing-2-ring-${formatNumber(this.boreDiameter)}-${formatNumber(
      this.outerDiameter
    )}-${formatNumber(this.width)}-${formatNumber(this.clearance)}.stl`;
  }
