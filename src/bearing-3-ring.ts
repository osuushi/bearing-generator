...
export class Bearing3Ring {
  ...
  get sliderOuterDiameter(): number {
    return (
      this.boreDiameter + (2 * (this.outerDiameter - this.boreDiameter)) / 3
    );
  }
  ...
  drawPreview(canvas: HTMLCanvasElement) {
    ...
    const polygons = [innerRing, slider, outerRing];
  ...
  get fileName(): string {
    ...
    return `bearing-3-ring-${formatNumber(this.boreDiameter)}-${formatNumber(
      this.outerDiameter
  ...
  validate() {
    if (
      this.boreDiameter >=
      this.sliderInnerDiameter - minThickness * 2 - this.clearance * 2
    ) {
